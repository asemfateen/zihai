...
export default function AchievementsPage() {
  ...
  const fetchAchievements = async () => {
    try {
      setLoading(true);
      const res = await fetchWithTimeout(`${API_BASE}/api/achievements`, {
        credentials: "include",
      });
    ...
    <div className="min-h-screen flex flex-col relative z-10 pb-24">
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-card/40 border border-border/50 rounded-[2.5rem] p-8 max-w-md mx-auto shadow-sm animate-fade-in">
            <Trophy className="w-12 h-12 text-orange-400/60 mx-auto mb-4" />
            <h3 className="text-lg font-bold text-text-primary mb-1">No Achievements</h3>
            <p className="text-text-secondary text-sm">Achievements are currently being initialized. Check back later!</p>
          </div>
        ) : (
